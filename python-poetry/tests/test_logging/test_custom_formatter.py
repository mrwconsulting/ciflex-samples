...
Stack (most recent call last):
  ...
  File "/FORTESTS/app/api/middlewares/try_except_middleware.py", line 15, in add_try_except
    logger.exception(msg, stack_info=True)
"""


def test_custom_json_formatter_with_stack_info(mocker):
    mocker.patch("app.config.Config.JSON_LOG_FORMAT", new=True)
    ...
    assert 'app/api/middlewares/try_except_middleware.py' in res
...
def test_custom_default_formatter_with_stack_info(mocker):
    mocker.patch("app.config.Config.JSON_LOG_FORMAT", new=False)
...
def test_custom_long_json_formatter_with_stack_info(mocker):
    mocker.patch("app.config.Config.JSON_LOG_FORMAT", new=True)
    mocker.patch("app.config.Config.LONG_LOG_LINE", new=True)
...
def test_custom_long_default_formatter_with_stack_info(mocker):
    ...
    m1 = 'app-test111 ERROR                                        Internal server error: division by zero'
    m2 = '  File "/FORTESTS/app/api/middlewares/try_except_middleware.py", line 15, in add_try_except'
